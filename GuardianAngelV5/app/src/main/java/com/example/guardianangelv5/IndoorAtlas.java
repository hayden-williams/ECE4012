...
public class IndoorAtlas {
    ...
    public final static double home_lat = 33.775752976752145;
    public final static double home_lon = -84.39689420163633;
        ...
        public void onLocationChanged(IALocation location) {
                ...
                switch (CurrentActivity.getInstance().getCurrentActivityNum()) {
                    case 1:
                        break;
                    case 2:
                        // display first recorded user location on map
                        if (!first_location_set[1]) {
                            ...
                            RequestScreen myRequestScreen = ((RequestScreen)app);
                            myRequestScreen.setMarker(Helper.updateMap(myRequestScreen.getMap(), myRequestScreen.getMarker(), myRequestScreen.getAtlas()));
                        }
                        first_location_set[1] = true;
                        break;
                    case 3:
                        // update user's location while they are waiting
                        Log.d(TAG, "Latitude: " + location.getLatitude());
                        lat = location.getLatitude();
                        Log.d(TAG, "Longitude: " + location.getLongitude());
                        lon = location.getLongitude();
                        ...
                        break;
                    case 4:
                        // update user's location during trip
                        Log.d(TAG, "Latitude: " + location.getLatitude());
                        lat = location.getLatitude();
                        Log.d(TAG, "Longitude: " + location.getLongitude());
                        lon = location.getLongitude();
                        Log.d(TAG, "Floor number: " + location.getFloorLevel());
                        floor = location.getFloorLevel();
                        TripScreen myTripScreen = ((TripScreen)app);
                        myTripScreen.setMarker(Helper.updateMap(myTripScreen.getMap(), myTripScreen.getMarker(), myTripScreen.getAtlas()));
                        break;
                    case 5:
                        // display first recorded user location on map
                        if (!first_location_set[4]) {
                            ...
                            Log.d(TAG, "Floor number: " + location.getFloorLevel());
                            floor = location.getFloorLevel();
                            EndScreen myEndScreen = ((EndScreen)app);
                            myEndScreen.setMarker(Helper.updateMap(myEndScreen.getMap(), myEndScreen.getMarker(), myEndScreen.getAtlas()));
                        }
            ...
            mLoadTarget = new Target() {
                ...
                public void onBitmapLoaded(Bitmap bitmap, Picasso.LoadedFrom from) {
                    ...
                    switch (CurrentActivity.getInstance().getCurrentActivityNum()) {
                        ...
                        case 2:
                            RequestScreen myRequestScreen = ((RequestScreen)app);
                            myRequestScreen.setOverlay(Helper.setupGroundOverlay(floorPlan, bitmap, myRequestScreen.getOverlay(), myRequestScreen.getMap()));
                            break;
                        case 3:
                            WaitScreen myWaitScreen = ((WaitScreen)app);
                            myWaitScreen.setOverlay(Helper.setupGroundOverlay(floorPlan, bitmap, myWaitScreen.getOverlay(), myWaitScreen.getMap()));
                            break;
                        case 4:
                            TripScreen myTripScreen = ((TripScreen)app);
                            myTripScreen.setOverlay(Helper.setupGroundOverlay(floorPlan, bitmap, myTripScreen.getOverlay(), myTripScreen.getMap()));
                            break;
                        case 5:
                            EndScreen myEndScreen = ((EndScreen)app);
                            myEndScreen.setOverlay(Helper.setupGroundOverlay(floorPlan, bitmap, myEndScreen.getOverlay(), myEndScreen.getMap()));
                            break;
                    }
                }
